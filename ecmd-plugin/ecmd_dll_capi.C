...
bool queryTargetConfigExist(const ecmdChipTarget& i_target,
...
//---------------------------------------------------------------------
uint32_t dllLoadDll(const char* i_clientVersion, uint32_t)
{
    uint32_t rc;
    ...
    uint32_t majorlength =
        (uint32_t)(strchr(i_clientVersion, '.') - i_clientVersion);
                       ...
                       ecmdLooperData& io_state, ecmdLoopMode_t i_mode)
{
    ...
    if (io_target.cageState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.cage = 0;
    if (io_target.nodeState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.node = 0;
    if (io_target.slotState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.slot = 0;
    if (io_target.chipTypeState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.chipType = "na";
    if (io_target.posState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.pos = 0;
    if (io_target.chipUnitTypeState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.chipUnitType = "na";
    if (io_target.chipUnitNumState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.chipUnitNum = 0;
    if (io_target.threadState == ECMD_TARGET_FIELD_WILDCARD)
        io_target.thread = 0;

    /* Set all the states to valid, unless they are unused */
    if (io_target.cageState != ECMD_TARGET_FIELD_UNUSED)
        io_target.cageState = ECMD_TARGET_FIELD_VALID;
    if (io_target.nodeState != ECMD_TARGET_FIELD_UNUSED)
        io_target.nodeState = ECMD_TARGET_FIELD_VALID;
    if (io_target.slotState != ECMD_TARGET_FIELD_UNUSED)
        io_target.slotState = ECMD_TARGET_FIELD_VALID;
    if (io_target.chipTypeState != ECMD_TARGET_FIELD_UNUSED)
        io_target.chipTypeState = ECMD_TARGET_FIELD_VALID;
    if (io_target.posState != ECMD_TARGET_FIELD_UNUSED)
        io_target.posState = ECMD_TARGET_FIELD_VALID;
    if (io_target.chipUnitTypeState != ECMD_TARGET_FIELD_UNUSED)
        io_target.chipUnitTypeState = ECMD_TARGET_FIELD_VALID;
    if (io_target.chipUnitNumState != ECMD_TARGET_FIELD_UNUSED)
        io_target.chipUnitNumState = ECMD_TARGET_FIELD_VALID;
    if (io_target.threadState != ECMD_TARGET_FIELD_UNUSED)
        io_target.threadState = ECMD_TARGET_FIELD_VALID;

    if (i_looptype == ECMD_ALL_TARGETS_LOOP)
    {
        if (i_mode == ECMD_EXIST_LOOP || i_mode == ECMD_EXIST_REVERSE_LOOP)
        {
            rc = dllQueryExist(queryTarget, io_state.ecmdSystemConfigData,
                               ECMD_QUERY_DETAIL_LOW);
        }
        else
        {
            rc = dllQueryConfig(queryTarget, io_state.ecmdSystemConfigData,
                                ECMD_QUERY_DETAIL_LOW);
        }
    }
    else
    {
        if (i_mode == ECMD_EXIST_LOOP || i_mode == ECMD_EXIST_REVERSE_LOOP)
        {
            rc = dllQueryExistSelected(
                queryTarget, io_state.ecmdSystemConfigData, i_looptype);
        }
        else
        {
        ...
        if (queryTarget.cageState == ECMD_TARGET_FIELD_UNUSED)
            io_target.cageState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.nodeState == ECMD_TARGET_FIELD_UNUSED)
            io_target.nodeState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.slotState == ECMD_TARGET_FIELD_UNUSED)
            io_target.slotState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.chipTypeState == ECMD_TARGET_FIELD_UNUSED)
            io_target.chipTypeState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.posState == ECMD_TARGET_FIELD_UNUSED)
            io_target.posState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.chipUnitTypeState == ECMD_TARGET_FIELD_UNUSED)
            io_target.chipUnitTypeState = ECMD_TARGET_FIELD_UNUSED;
        if (queryTarget.chipUnitNumState == ECMD_TARGET_FIELD_UNUSED)
            io_target.chipUnitNumState = ECMD_TARGET_FIELD_UNUSED;
     ...
     * structures */
    if (i_mode == ECMD_EXIST_REVERSE_LOOP ||
        i_mode == ECMD_CONFIG_REVERSE_LOOP)
    {
        ...
        for (curCage = io_state.ecmdSystemConfigData.cageData.begin();
             curCage != io_state.ecmdSystemConfigData.cageData.end();
             curCage++)
        {
                       ...
                       ecmdLoopMode_t i_mode)
{
    ...
    {
        dllOutputError(
            "ecmdConfigLooperNext - Invalid io_state passed, verify ecmdConfigLooperInit was run successfully\n");
    ...
    if (io_state.ecmdUseUnitid)
    {
        ...
        while (!done)
        {
            ...
            {
                if (dllQueryTargetExist(io_target,
                                        &(io_state.ecmdSystemConfigData)))
                {
                    done = true;
                }
            }
            ...
            {
                if (dllQueryTargetConfigured(io_target,
                                             &(io_state.ecmdSystemConfigData)))
                {
                    done = true;
                }
    ...
    else
    {
        while (!done)
        {
             ...
             * of here */
            if (io_state.ecmdCurCage ==
                io_state.ecmdSystemConfigData.cageData.end())
            {
                return rc;
            }

            /* ******** NOTE : The iterators in io_state always point to the
                ...
                io_target.cage != (*io_state.ecmdCurCage).cageId)
            {
                ...
                else if ((io_state.prevTarget.nodeState !=
                          ECMD_TARGET_FIELD_UNUSED) &&
                         (io_state.ecmdCurNode ==
                          (*io_state.ecmdCurCage).nodeData.end()))
                {
            ...
            if (level == NODE &&
                (!valid || io_target.node != (*io_state.ecmdCurNode).nodeId))
            {
                /* Data is valid, let's setup this part of the target */
                ...
                else if ((io_state.prevTarget.slotState !=
                          ECMD_TARGET_FIELD_UNUSED) &&
                         (io_state.ecmdCurSlot ==
                          (*io_state.ecmdCurNode).slotData.end()))
                {
                    /* Increment the iterators to point to the next target (at
                     * the level above us) */
                    ecmdIncrementLooperIterators(level - 1, io_state);
            ...
            if (level == SLOT &&
                (!valid || io_target.slot != (*io_state.ecmdCurSlot).slotId))
            {
                /* Data is valid, let's setup this part of the target */
                ...
                /* If next level is unused we default to 0 */
                if ((io_state.prevTarget.chipTypeState ==
                         ECMD_TARGET_FIELD_UNUSED ||
                     io_state.prevTarget.posState == ECMD_TARGET_FIELD_UNUSED))
                {
                    /* If the next level is required but empty, this position
                     * isn't valid we need to restart */
                }
                else if ((io_state.prevTarget.chipTypeState ==
                              ECMD_TARGET_FIELD_UNUSED ||
                          io_state.prevTarget.posState ==
                              ECMD_TARGET_FIELD_UNUSED) &&
                         (io_state.ecmdCurChip ==
                          (*io_state.ecmdCurSlot).chipData.end()))
                {
                    /* Increment the iterators to point to the next target (at
                     * the level above us) */
                    ecmdIncrementLooperIterators(level - 1, io_state);
                 ...
                 io_target.pos != (*io_state.ecmdCurChip).pos))
            {
                ...
                io_state.ecmdCurChipUnit =
                    (*io_state.ecmdCurChip).chipUnitData.begin();
                valid = 0;

                /* If next level is unused we default to 0 */
                if ((io_state.prevTarget.chipUnitNumState ==
                     ECMD_TARGET_FIELD_UNUSED))
                {
                    /* If the next level is required but empty, this position
                     * isn't valid we need to restart */
                }
                else if ((io_state.prevTarget.chipUnitNumState !=
                          ECMD_TARGET_FIELD_UNUSED) &&
                         (io_state.ecmdCurChipUnit ==
                          (*io_state.ecmdCurChip).chipUnitData.end()))
                {
                    /* Increment the iterators to point to the next target (at
                     * the level above us) */
                    ecmdIncrementLooperIterators(level - 1, io_state);
            ...
            if (level == CHIPUNIT &&
                (!valid ||
                 io_target.chipUnitType !=
                     (*io_state.ecmdCurChipUnit).chipUnitType ||
                 io_target.chipUnitNum !=
                     (*io_state.ecmdCurChipUnit).chipUnitNum))
            {
                /* Data is valid, let's setup this part of the target */
                io_target.chipUnitType =
                    (*io_state.ecmdCurChipUnit).chipUnitType;
                ...
                io_state.ecmdCurThread =
                    (*io_state.ecmdCurChipUnit).threadData.begin();
                valid = 0;
                ...
                else if ((io_state.prevTarget.threadState !=
                          ECMD_TARGET_FIELD_UNUSED) &&
                         (io_state.ecmdCurThread ==
                          (*io_state.ecmdCurChipUnit).threadData.end()))
                {
                    /* Increment the iterators to point to the next target (at
                     * the level above us) */
                    ecmdIncrementLooperIterators(level - 1, io_state);
                                  ...
                                  ecmdLoopType_t i_looptype, bool i_existMode)
{
    ...
    if ((i_looptype == ECMD_SELECTED_TARGETS_LOOP_VD) ||
        (i_looptype == ECMD_SELECTED_TARGETS_LOOP_VD_DEFALL))
    {
